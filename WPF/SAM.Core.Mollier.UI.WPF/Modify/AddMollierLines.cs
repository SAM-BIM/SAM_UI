...
namespace SAM.Core.Mollier.UI
{
    ...
    {
        public static List<Series> AddMollierLines(this PlotModel plotModel, IEnumerable<UIMollierCurve> uIMollierCurves, MollierControlSettings mollierControlSettings)
        {
            ...
            // Find the 100% relative-humidity (saturation) curve already added to the model.
            Polyline2D polyline2D_RelativeHumidity = null;
            foreach (Series series_Temp in plotModel.Series)
            {
                LineSeries lineSeries = series_Temp as LineSeries;
                ConstantValueCurve constantValueCurve = lineSeries?.Tag as ConstantValueCurve;
                ...
                {
                    polyline2D_RelativeHumidity = new Polyline2D(lineSeries.Points.ConvertAll(x => x.ToSAM()));
                    break;
                }
            }
            ...
            // Chart display bounds from the configured axes (set by the host control before population),
            // the OxyPlot equivalent of WinForms chart.ChartAreas[0].Axis{X,Y}.Minimum/Maximum.
            Axis xAxis = plotModel.Axes.FirstOrDefault(a => a.IsHorizontal());
            Axis yAxis = plotModel.Axes.FirstOrDefault(a => a.IsVertical());
            if (xAxis == null || yAxis == null)
            {
                return null;
            }

            BoundingBox2D boundingBox2D = new BoundingBox2D(new Point2D(xAxis.Minimum, yAxis.Minimum), new Point2D(xAxis.Maximum, yAxis.Maximum));
            ...
            foreach (UIMollierCurve uIMollierCurve in uIMollierCurves)
            {
                ...
                System.Drawing.Color color = uIMollierCurve.UIMollierAppearance.Color == System.Drawing.Color.Empty ? System.Drawing.Color.Gray : uIMollierCurve.UIMollierAppearance.Color;
                ...
                foreach (Segment2D segment2D in segment2Ds)
                {
                    ...
                    {
                        Title = "Mollier Sensible Heat Ratio Line " + Guid.NewGuid().ToString(),
                        ...
                        Tag = uIMollierCurve,
                        TrackerFormatString = Query.ToolTipText(mollierSensibleHeatRatioLine, chartType),
                    };
                    series.Points.Add(new DataPoint(segment2D[0].X, segment2D[0].Y));
                    series.Points.Add(new DataPoint(segment2D[1].X, segment2D[1].Y));
        ...
        public static List<Series> AddMollierLines(this PlotModel plotModel, MollierModel mollierModel, MollierControlSettings mollierControlSettings)
        {
            ...
            return plotModel.AddMollierLines(uIMollierCurves, mollierControlSettings);
        }
    }
}
