...
- (Codex P2 x2 on 91085dc, fixed after this run: the "Starting from" line for a design an earlier 2B left behind said "numbering continues from it" (as captured in 6), but only the result files continue - the new run's rounds are shown from 1. It now reads "saved as round 10 (-Opt10)", and its tooltip says the rounds count from 1 while the files continue from -Opt11. The Start window is also capped to the work area with scrolling content, so Start and Cancel stay reachable.)
